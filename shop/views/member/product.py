...
def product_list(request):
    ...
    # Default SQL query
    sql = """
        SELECT DISTINCT P.PID, P.PName, P.Category, P.Stock, P.Price,
               EXISTS (
        ...
        LEFT JOIN (SELECT PID, SUM(Quantity) AS SoldCount FROM ORDER_DETAIL GROUP BY PID) AS Sold ON P.PID = Sold.PID
    """
    # Join with sold products to get the sold count
    sql += " LEFT JOIN (SELECT PID, SUM(Quantity) AS SoldCount FROM ORDER_DETAIL GROUP BY PID) AS Sold ON P.PID = Sold.PID"
...
def product_detail(request, pid):
    ...
    sql = """
        SELECT P.PID, P.PName, P.Category, P.Price, P.Stock, P.Descript, S.SName AS SellerName,
               Pr.PromoCode, Pr.DisAmount
        FROM PRODUCT P
