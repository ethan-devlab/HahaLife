...
def add_product(request):
    ...
    if request.method == 'POST':
        ...
        execute_query(
            """
             INSERT INTO PRODUCT (PID, PName, Category, Price, Descript, Stock, SID)
             VALUES (%s, %s, %s, %s, %s, %s, %s)
             """,
            (pid, pname, category, price, description, stock, sid)
        )
        # Insert tags
...
def manage_products(request):
    ...
    sql = """
        ...
        WHERE P.SID = %s
        GROUP BY P.PID, P.PName, P.Category, P.Price, P.Stock
    """
...
def product_detail_s(request, pid):
    ...
    product = execute_query(
        ...
        LEFT JOIN PROMOTION Pr ON HP.PromoCode = Pr.PromoCode
        WHERE P.PID = %s
        GROUP BY P.PID, P.PName, P.Category, P.Price, P.Stock, P.Descript, S.SName
        """,
        (pid,), fetch=True
    )[0]
...
def edit_product(request, pid):
    ...
    prod = execute_query("SELECT * FROM PRODUCT WHERE PID = %s AND SID = %s", (pid, sid), fetch=True)
    if not prod:
    ...
    if request.method == 'POST':
        ...
        execute_query(
            "UPDATE PRODUCT SET PName=%s, Category=%s, Price=%s, Descript=%s, Stock=%s WHERE PID=%s",
            (pname, category, price, description, stock, pid)
        )
        # update tags
